...
*   **Didático e Simples: Um Laboratório de Sistemas de Arquivos:** A maior vantagem é a clareza. A separação entre a lógica (`fs.c`) e a interface (`shell.c`) permite que um estudante se concentre em entender a manipulação da árvore sem se distrair com o parsing de entrada. O código é procedural e usa estruturas de dados fundamentais, tornando-o acessível e, ao mesmo tempo, um excelente exemplo de engenharia de software em C.
...
*   **Não Gerencia Disco Real:** Esta é a principal simplificação. Implementar o gerenciamento de blocos em disco introduziria uma camada de complexidade (drivers, acesso a setores, bitmaps de alocação) que desviaria o foco do objetivo principal: entender a estrutura de dados em árvore.
